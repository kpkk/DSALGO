package SecondWeekHomeWork;

import org.junit.Test;

import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;

public class FindAllSubArraysMatchingTarget_SlidingWindow {

    /*

     Find all subarrays in the input array matching the target (try solving using hashing technique also)
Example:
Input: [1, 7, 4, 3, 4, 1, 2, 5, 1]
target: 7
Output:
1,1
2,3
3,4
4,6
6,7

    --------------------------------------------------------------
    Problem solving Template

    a) Did i under stand the question- Yes
    b) Do I know what is input, output and constraints
  Input: [1, 7, 4, 3, 4, 1, 2, 5, 1]
target: 7
    Constraints: use sliding window technique

    c) prepare test data (Minimum of 3 set)


      d ) Write the Pseudo code
     */

    @Test
    public void example1(){
        int[] nums={1, 7, 4, 3, 4, 1, 2, 5, 1};
        int target=7;
        returnSubArrays(nums,target);

    }

    private void returnSubArrays(int[] nums, int target) {
        List<String> output = new ArrayList<String>();

        int start = 0, end = 0;
        int sum = 0;
        for (; start < nums.length; start++) {
            while (sum < target && end < nums.length)
                sum += nums[end++];

            if (sum == target)
                output.add(Arrays.toString(new int[] { start, end-1 }));

            sum -= nums[start];
        }

        System.out.println(output);
    }

//    private int[] findSubArray(int []nums, int target){
//
//        for (int i=0;i<nums.length;i++){
//            if(nums[i]==target){
//                return new int[]{i,i};
//            }else{
//                nums[i]=nums[i++];
//                if (nums[i]==target){
//                    return new int[]{i,i++};
//                }
//            }
//
//        }
//
//    }
}
